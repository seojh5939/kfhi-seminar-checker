...
- [ ] 프로젝트 최상위 루트의 `google-credentials.json`을 정상 감지하는지 확인
- [ ] 구글 로그인 클릭 시 기본 브라우저가 열리고 조직 계정 로그인 후 토큰이 정상 저장되는지 확인
- [ ] 내 드라이브의 스프레드시트 목록이 드롭다운에 표시되고 새 시트 생성이 정상 작동하는지 확인
- [ ] 장소명을 `입구`로 설정 시 구글 시트에 `입구` 탭이 생성되고 7개 컬럼 헤더가 세팅되는지 확인
- [ ] QR 스캔 시 해당 장소 탭에 실시간으로 데이터가 추가되는지 확인
- [ ] 장소명을 `행복한나눔`으로 변경 시 `행복한나눔` 탭이 자동 생성되고 해당 탭으로 기록되는지 확인
- [ ] 네트워크 연결 해제 시에도 로컬에 정상 저장되고, 재연결 시 큐에 대기 중이던 기록이 일괄 업로드되는지 확인
